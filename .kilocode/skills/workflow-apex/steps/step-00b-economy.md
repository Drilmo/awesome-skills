...
description: Economy mode overrides - no subtasks, direct tool usage to save tokens
...
These instructions OVERRIDE the default behavior in all steps to save tokens by avoiding subtask launches.
...
- ✅ Faster execution (no subtask overhead)
...
### Override 1: No Subtask Launches

**DEFAULT behavior (when economy_mode = false):**
```
Launch parallel subtasks via new_task:
- Subtask 1: explore-codebase mode
- Subtask 2: explore-docs mode
- Subtask 3: websearch mode
```
...
**NEVER use new_task tool in economy mode.**

---

### Override 2: Direct Tool Usage Pattern

Instead of launching exploration subtasks, use this pattern:
...
**DEFAULT:** Launch code-reviewer subtask for adversarial review
**ECONOMY:** Self-review without subtask
...
3. Quick self-review checklist:
   ...
   - [ ] No security issues
4. Skip adversarial review subtask
```
...
```
INSTEAD OF: 3 parallel exploration subtasks
DO:
...
```
Same as default - planning doesn't use subtasks
```

### Step 03: Execute (Economy)
```
Same as default - execution doesn't use subtasks
```
...
```
INSTEAD OF: code-reviewer subtask
DO:
...
```
INSTEAD OF: Deep test analysis with subtasks
DO:
...
```
Same as default - test running doesn't use subtasks
```
...
```
⚡ ECONOMY MODE - Using direct tools, no subtasks
```

This reminds both AI and the user that economy mode is active.
</economy_indicator>
...
- No new_task calls to create subtasks
