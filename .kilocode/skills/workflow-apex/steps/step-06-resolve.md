...
Options:
- Auto-fix Real issues (Recommended): Fix 'Real' findings, skip noise/uncertain
- Walk through each finding: Decide on each finding individually
- Fix only critical: Only fix CRITICAL/BLOCKING issues
- Skip all: Acknowledge but don't change
```
...
Use `ask_followup_question`:
```
How should we handle finding F1?

Options:
- Fix now (Recommended): Apply the suggested fix
- Skip: Acknowledge but don't fix
- Discuss: Need more context
- Mark as noise: Not a real issue
```
...
Use `ask_followup_question`:
```
You have unresolved Critical/High findings. Proceed anyway?

Options:
- Go back and fix: Return to resolution options
- Proceed anyway: Accept risks, continue
- Fix only critical: Just fix critical issues
```
...
❌ **CRITICAL**: Not using `ask_followup_question` for decisions
