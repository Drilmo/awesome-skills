...
- 🎯 Launch parallel review subtasks (unless economy_mode)
...
| `{economy_mode}` | No subtasks, direct review |
...
→ Launch parallel review subtasks using `new_task`

**CRITICAL: Create ALL subtasks for parallel execution:**

**Subtask 1: Security** (use `new_task` in **code-reviewer** mode)
```
...
**Subtask 2: Logic** (use `new_task` in **code-reviewer** mode)
```
Review for:
...
Focus: Logic and correctness issues only
```

**Subtask 3: Clean Code** (use `new_task` in **code-reviewer** mode)
```
Review for:
- SOLID violations
- Code smells
...
Focus: Code quality issues only
```

**Subtask 4: Vercel/Next.js Best Practices** (CONDITIONAL)
...
Use `new_task` in **code-reviewer** mode with focus on:
...
→ **If NOT Next.js/Vercel code:** Skip this subtask
...
Options:
- Resolve findings (Recommended): Address the identified issues
- Skip to tests: Skip resolution, proceed to test creation
- Skip resolution: Accept findings, don't make changes
- Discuss findings: I want to discuss specific findings
```
...
❌ Using subtasks when economy_mode
❌ Skipping Vercel/Next.js review when React/Next.js files are modified
❌ **CRITICAL**: Not using `ask_followup_question` for review → resolve/test transition
...
After user confirms via `ask_followup_question` (or auto-proceed):
