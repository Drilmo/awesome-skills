...
- 🚫 FORBIDDEN to use Edit, Write, or command tools
...
```
Multiple approaches are possible. Which should we use?

Options:
- Approach A (Recommended): Description and tradeoffs of A
- Approach B: Description and tradeoffs of B
- Approach C: Description and tradeoffs of C
```
...
Use `ask_followup_question`:
```
Review the implementation plan. Ready to proceed?

Options:
- Approve and execute (Recommended): Plan looks good, start implementation
- Adjust plan: I want to modify specific parts
- Ask questions: I have questions about the plan
- Start over: Revise the entire plan
```
...
❌ **CRITICAL**: Not using `ask_followup_question` for approval
...
After user approves via `ask_followup_question` (or auto-proceed), load `./step-03-execute.md`
