...
```
Tests require services that aren't running. How proceed?

Options:
- I'll start manually: Give me a moment to start them
- Start automatically: Try to start services automatically
- Skip tests needing services: Only run tests that don't need services
- Skip test step: Continue without running tests
```
...
Use `ask_followup_question`:
```
Test keeps failing. How proceed?

Options:
- I'll debug manually: Let me investigate
- Skip this test: Mark as skip, continue others
- Delete test: Remove this test entirely
- Keep trying: Try more approaches
```
...
Use `ask_followup_question`:
```
Configuration issue detected. How proceed?

Options:
- I'll fix manually: Let me handle config
- Try automatic fix: Attempt suggested fix
- Skip tests: Continue without tests
```
...
Use `ask_followup_question`:
```
All tests passing. What next?

Options:
- Run adversarial review: Deep review for security/logic
- Complete workflow: Finalize and show summary
```
...
❌ **CRITICAL**: Not using `ask_followup_question` when stuck
