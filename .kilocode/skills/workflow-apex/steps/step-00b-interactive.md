...
- ✅ ALWAYS use ask_followup_question for flag selection
...
| Economy (`-e`) | {economy_mode ? "✓ ON" : "✗ OFF"} | No subtasks |
...
Use `ask_followup_question` with multiSelect:
```
Select flags to TOGGLE (currently shown flags will flip their state):

Options (multi-select):
- Auto mode: {auto_mode ? 'Disable' : 'Enable'} - skip confirmations
- Examine mode: {examine_mode ? 'Disable' : 'Enable'} - adversarial review at end
- Save mode: {save_mode ? 'Disable' : 'Enable'} - save outputs to .claude/output/
- Test mode: {test_mode ? 'Disable' : 'Enable'} - include test creation/runner
```
...
Use `ask_followup_question` with multiSelect:
```
Select additional flags to TOGGLE:

Options (multi-select):
- Economy mode: {economy_mode ? 'Disable' : 'Enable'} - no subtasks, save tokens
- Branch mode: {branch_mode ? 'Disable' : 'Enable'} - verify/create git branch
- PR mode: {pr_mode ? 'Disable' : 'Enable'} - create pull request at end
- Done - keep current: No more changes, proceed with workflow
```
...
❌ **CRITICAL**: Using plain text prompts instead of `ask_followup_question`
