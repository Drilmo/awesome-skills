...
**Implementation (Kilocode):**

```markdown
<parallel_execution>
**Create ALL subtasks using `new_task`:**

Subtask 1: Use `new_task` in **explore-codebase** mode
→ Find files related to the task.

Subtask 2: Use `new_task` in **explore-docs** mode
→ Research documentation for the task.

Subtask 3: Use `new_task` in **websearch** mode
...
**Note:** Skills that spawn subtasks must be run in **orchestrator** mode.
...
IF {examine_mode} = true OR user_requests:
    1. Use `new_task` in **code-reviewer** mode
    2. Collect findings
...
### CRITICAL: Orchestrator mode requirement

Skills that spawn subtasks must be run in **orchestrator** mode. Add this preamble:
