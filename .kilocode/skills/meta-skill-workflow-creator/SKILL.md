---
name: meta-skill-workflow-creator
description: Create multi-step workflow skills with progressive step loading, state management, and interactive decisions. Use when building complex skills like APEX that need structured phases, when asked to "create a workflow skill", "build a multi-step skill", or "design a skill with steps".
---
...
- [Ask Patterns](references/ask-patterns.md) - ask_followup_question patterns
...
| Mechanism | Use When |
|-----------|----------|
| `new_task` | Need isolated context, parallel subtasks, independent execution |
| `switch_mode` | Change behavior in current conversation, sequential workflow |
...
→ ALWAYS use ask_followup_question tool for ANY user input
...
→ Check auto_mode before ANY ask_followup_question call
