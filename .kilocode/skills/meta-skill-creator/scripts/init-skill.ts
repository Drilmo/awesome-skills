 ...
 *
 * Examples:
 *   bun scripts/init-skill.ts my-new-skill --path ~/.kilocode/skills
 *   bun scripts/init-skill.ts my-api-helper --path .kilocode/skills
 */
...
Asset files are NOT loaded into context - they're used in the AI's output.
...
if (import.meta.main) {
  ...
  if (args.length < 3 || args[1] !== "--path") {
    ...
    console.log("\nExamples:");
    console.log("  bun scripts/init-skill.ts my-skill --path ~/.kilocode/skills");
    console.log("  bun scripts/init-skill.ts api-helper --path .kilocode/skills");
    process.exit(1);
  }
