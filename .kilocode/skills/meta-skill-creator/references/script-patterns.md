...
<when_to_use_scripts>
Even if the AI could write a script, pre-made scripts offer advantages:
...
Make clear whether the AI should:
...
<how_scripts_work>
When the AI executes a script via bash:

1. Script code never enters context window
2. Only script output consumes tokens
3. Far more efficient than having the AI generate equivalent code
</how_scripts_work>
</when_to_use_scripts>
...
```bash
python ~/.kilocode/skills/skill-name/scripts/analyze.py input.har
```
</reference_pattern>
</directory_organization>
...
<error_handling>
<solve_dont_punt>
Handle error conditions rather than punting to the AI.
...
```python
def process_file(path):
    # Just fail and let the AI figure it out
...
- **Most environments**: Can install packages from npm and PyPI
- **Restricted environments**: May have no network access or no runtime package installation
</runtime_constraints>
